...
import type { EdgarCoreFinancialsResponse, EdgarIncomeStatementConsolidated, EdgarIncomeStatementPayload, EdgarIncomeStatementValue } from '../types/edgar_deprecated';

dotenv.config();

class EdgarDeprecatedClient {
  constructor(
  ...
  private static formatIncomeStatements(incomeStatements: EdgarIncomeStatementConsolidated[], symbol: string) {
    return incomeStatements.map(({ rownum, values }) => ({
      ...
      cost_of_revenue: EdgarDeprecatedClient.getRoundedValue(values, 'costofrevenue'),
      ebit: EdgarDeprecatedClient.getRoundedValue(values, 'ebit'),
      eps: values.find(({ field }) => field === 'basicepsnetincome')?.value ?? 0,
      eps_diluted: values.find(({ field }) => field === 'dilutedepsnetincome')?.value ?? 0,
      gross_profit: EdgarDeprecatedClient.getRoundedValue(values, 'grossprofit'),
      income_tax_expense: EdgarDeprecatedClient.getRoundedValue(values, 'incometaxes'),
      net_income: EdgarDeprecatedClient.getRoundedValue(values, 'netincome'),
      net_income_com: EdgarDeprecatedClient.getRoundedValue(values, 'netincomeapplicabletocommon'),
      operating_income: EdgarDeprecatedClient.getRoundedValue(values, 'operatingprofit'),
      rd_expense: EdgarDeprecatedClient.getRoundedValue(values, 'researchdevelopmentexpense'),
      revenue: EdgarDeprecatedClient.getRoundedValue(values, 'totalrevenue'),
      sga_expense: EdgarDeprecatedClient.getRoundedValue(values, 'sellinggeneraladministrativeexpenses'),
  ...
  public async getIncomeStatements(equities: string[]) {
    ...
    for (let i = 0; i < equities.length; i += 1) {
      ...
      if (Array.isArray(rows)) {
        formattedIncomeStatements = EdgarDeprecatedClient.formatIncomeStatements(rows, currentSymbol);
      }
...
export default EdgarDeprecatedClient;
