...
import { RECENT_FINANCIALS_HEADERS } from '../constants/headers';
...
const recentFinancialsModel = {
  async saveAll(recentFinancials: RecentFinancialsPayload[]) {
    const recentFinancialsCsvClient = new CSVClient(PATH_TO_RECENT_FINANCIALS_CSV as string, RECENT_FINANCIALS_HEADERS as string[]);
